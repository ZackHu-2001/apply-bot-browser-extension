...
class TabShareExtension {
  ...
  private _onMessage(message: PageMessage, sender: chrome.runtime.MessageSender, sendResponse: (response: any) => void) {
    switch (message.type) {
      ...
      case 'connectToCurrentTab':
        // Handle connection from content script (Dashboard)
        chrome.tabs.query({ active: true, currentWindow: true }).then(tabs => {
          ...
          // First connect to relay, then connect the tab
          this._connectToRelay(-1, message.mcpRelayUrl).then(() => {
            const connection = this._pendingTabSelection.get(-1)?.connection;
            if (!connection) {
              sendResponse({ success: false, error: 'Failed to establish relay connection' });
              return;
            }

            // Move connection to active
  ...
  private _onExternalMessage(message: ExternalMessage, sender: chrome.runtime.MessageSender, sendResponse: (response: any) => void) {
    debugLog('Received external message:', message, 'from:', sender.origin);

    // Verify the sender is from localhost
    ...
    switch (message.type) {
      ...
      case 'connectToCurrentTab':
        // Get the current active tab and connect to it
        chrome.tabs.query({ active: true, currentWindow: true }).then(tabs => {
          if (!tabs[0]?.id) {
            sendResponse({ success: false, error: 'No active tab found' });
            return;
